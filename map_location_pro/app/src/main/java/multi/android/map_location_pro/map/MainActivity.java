 ...
 * 이 위치를 찾는데 시간이 걸린다. 따라서 내부에서 자동을 전달될 수 있도록 작업 - v2로 바뀌면서 적용된 내용
 * 0. FragmentManager를 이용해서 SupportMapFragment를 find
 * 1. OnMapReadyCallback을 구현하고 onMapReady 메소드를 오버라이딩
 * 2. SupportMapFragment 객체에 getMapAsync 메소드를 이용해서 1번에서 구현한 OnMapReadyCallback 객체를 연결
 * 3. 맵이 준비되었을때 자동으로 onMapReady 메소드가 호출되면서 매개변수로 구글맵이 전달된다.
 */
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Map 프레그먼트를 추출
        //현재 xml 문서에 정의된 Fragment를 추출하는 경우 FragmentManager를 이용해서 추출
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if (map != null){
            ...
            //구글맵이 v2가 되면 카메라 개념이 추가
            //지도는 카메라로 아래를 내려다보는 듯한 내용이 모델링
            //변경사항에 관련된 내용을 담고 있는 객체 - CameraUpdate
            //CameraUpdate 객체에 변경할 값들을 셋팅해서 매개변수로 전달
            //CameraUpdate 객체를 만드는 객체
            //CameraUpdateFactory 객체의 여러 메소드를 통해서 CameraUpdate 객체를 생성
    ...
    public void setPosition(View view){
        LatLng myloc = new LatLng(37.5001621,127.0378924);
    ...
    public void addCircle(View view){
        ...
        circleOptions.fillColor(Color.parseColor("#550000ff")); //55는 투명도 뒤에 색상
        circleOptions.center(new LatLng(37.5001621,127.0378924)); //circle의 중심
